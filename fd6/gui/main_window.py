...
class MainWindow(QMainWindow):
    ...
    def _show_about(self) -> None:
        QMessageBox.about(
            ...
            "auto-\"updated\" people DOWN to the original build. The updater now "
            "only recognizes real X.Y.Z app versions and ignores build numbers, "
            "so it always lands on the genuine newest release.<br>"
            "<b>If you were downgraded, install this build manually once</b> from "
            "the GitHub releases page — the old build's updater can't recover on "
            "its own.<br><br>"
            "<b>v0.5.3:</b> fixed the post-update \"Failed to load Python DLL\" "
            "crash (clean relaunch + auto-launch after updating); update check no "
            "longer falsely reports \"up to date\"; Rich Presence shows the "
            "version; no re-link nag for linked users.<br>"
